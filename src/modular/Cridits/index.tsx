...
const CreditsScreen = (props: Props) => {
  ...
  const fetchPaymentSheetParams = async () => {
    const { paymentIntent, ephemeralKey, customer } = {
      paymentIntent:  "pi_3MC1K2JPH93PAwz91MXgwLzw_secret_LBXgSNoGsIvfeZBZicjui0fgi",
      ephemeralKey: "ek_test_YWNjdF8xTTUydVpKUEg5M1BBd3o5LDRHZ3dZY1Jxd3FnNklGSVFsZWJoRnExaGV2bFF3N0Y_00rNAWh5G3",
      customer: "cus_MvsbSEAcJgmD0K"
    }//await response.json();
  ...
  const initialisePaymentSheet = async () => {
    ...
    try {
      ...
      if (!error) {
        // setalert({ ...alert, message:" error.message", type: "error", showCancelButton: true, onConfairm: () => { setShowAlert(false) } })
        // setShowAlert(true)
        setPaymentSheetEnabled(true);
      } else {

        setalert({ ...alert, message: error.message, suTitle: undefined, type: "error", showCancelButton: false, onConfairm: () => { setShowAlert(false) } })
        setShowAlert(true)
  ...
  const onPressBuy = async () => {
    ...
    if (error) {
      ...
      setalert({ ...alert, message: 'The payment was confirmed successfully!', type: "success", showCancelButton: false, onConfairm: () => { setShowAlert(false) ; navigation.navigate("Home") } })
      setShowAlert(true)
